...
\title{{\bfseries\rmfamily\Huge{高等流体力学\hspace{1em}\\第4次作业}}}
...
\section{非定常Burgers涡精确解}

认为柱坐标下速度满足：
$$
    V_r=V_r(r),\ V_\theta = V_\theta(r,t),\ V_z=V_z(r,z)
$$

则连续性方程简化为：
$$
    \pd{rV_r}{r} + \pd{rV_z}{z}=0
$$

周向动量方程为：
$$
    \pd{V_\theta}{t}+V_r \pd{V_\theta}{r} + \frac{V_rV_\theta}{r}
    =
    \nu \left(
    \pd{^2V_\theta}{r^2} + \frac{1}{r}\pd{V_\theta}{r}
    -
    \frac{V_\theta}{r^2}
    \right)
$$
...
$$
    \pd{^2\Gamma}{r^2} =
    \left(\frac{1}{r} + \frac{V_r}{\nu}\right)\frac{d\Gamma}{dr}
    +
    \frac{1}{\nu}\pd{\Gamma}{t}
$$

此时，根据Burgers涡的假设，先根据连续性方程推导径向和轴向速度，
设为一个简单的线性分布：
$$
V_z = 2az, V_r = -ar
$$
...
接下来考虑周向速度。周向速度在定常下的解是
$$
\Gamma = 2\pi r V_\theta = \Gamma_0\left(
    1 - \exp\left(-\frac{ar^2}{2\nu}\right)
\right)
$$
不妨设非定常下有形式：
...
F(\tau) = F(at)
$$
是无量纲函数，$\tau$是无量纲时间。代入非定常的环量方程中有：
$$
\frac{a^2r^2}{2\nu^2}
\exp\left(-\frac{ar^2}{2\nu} F(\tau)\right)
\left[\frac{dF(\tau)}{d\tau}-2(F(\tau)-F(\tau)^2)\right]=0
$$
使其在$r\neq \infty, r\neq 0$的地方都成立，则一定有：
$$
\frac{dF}{d\tau} = 2(F - F^2)
$$
给定一个$F$的初值，即可确定其时间方向的变化规律。

很明显，$F=1$是对应定常Burgers涡的解。$F=0$是另一个定常解，
对应的是一个驻点流动。

从稳定性角度分析，$F=1$附近Jacobian是负特征值，为线性稳定
的零点；0附近则相反。容易发现，任何小于0的初值都会迅速趋向于
负无穷大，是一个“爆炸”的解。很容易理解的是，当$F<0$，速度空间
分布其实在无穷远是无穷大的，因此这是一个“非法”的初值。
因此只考虑$F(0)>0$的初值，
事实上可解得：\
当$F(0)>1$：
\begin{equation}\label{eq:F0}
    F=\frac{1}{1-\exp(C-2\tau)}
\end{equation}
其中有
$$
C = \ln{\left(1-\frac{1}{F(0)}\right)}
$$
当$0<F(0)<1$：
...
其中有
$$
C = \ln{\left(\frac{1}{F(0)}-1\right)}
$$
这些都是趋近于$F=1$的解。

综合以上的分析，形式为\eqref{eq:solution}的非定常Burgers涡，
可解得当$F(0)>0$的解析解为\eqref{eq:F0}以及\eqref{eq:F1}，其都是
趋近于定常解$F=1$，同时也反映了非定常解的稳定性范围。
