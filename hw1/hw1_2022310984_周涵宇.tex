...
\title{{\bfseries\rmfamily\Huge{高等流体力学\hspace{1em}\\第1次作业}}}
...
\section{Rayleigh-Bernard热对流的理解}

\label{sec:1}

根据近似，有流函数：
$$
    \psi=\psi_0\sin(a\pi x)\sin(\pi y)
$$
温度差
$$
    \theta=\theta_0\cos(a\pi x)\sin(\pi y)
$$

则有速度场：
$$
    u=-\pi\psi_0\sin(a\pi x)\cos(\pi y),\ \
    v=a\pi\psi_0\cos(a\pi x)\sin(\pi y)
$$

取$a=1.4, \psi_0=1$，绘制$[0,2/a]\times[0,1]$的流线：

\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{A14.png}  %需调整
    \caption{$a=1.4$流线}
    \label{fig:a}
\end{figure}
...
考虑运动学边界条件，当$y=0,1$，有$v=0$，明显满足；
动力学边界条件，
当$y=0,1$，有$\pd{u}{y}=\pi^2\psi_0\sin(a\pi x)\sin(\pi y)=0$
可知也满足。

因此这个速度场满足边界条件。$\theta$在上下边界为0，意味着总温度满足给定温度边界条件。
